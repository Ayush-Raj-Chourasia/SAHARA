...
// ─── Landing & Login Components (Moved up to fix hoisting) ───────────────────
...
const LoginPage = ({ onLogin, onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-white text-[#111827] font-['Outfit',sans-serif] flex flex-col items-center justify-center p-6 selection:bg-[#EA580C] selection:text-white">
      <div className="fixed inset-0 pointer-events-none opacity-20"><div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] bg-gradient-to-br from-[#EA580C]/10 to-transparent blur-[120px] rounded-full" /></div>
      <button onClick={onBack} className="absolute top-10 left-10 p-2 hover:bg-gray-100 rounded-full transition-colors flex items-center gap-2 font-bold text-sm"><LoginIcons.ArrowLeft w={20} /> Back</button>
      <div className="w-full max-w-md">
        <div className="flex flex-col items-center mb-12">
          <div className="p-4 rounded-[24px] bg-[#111827] text-white shadow-2xl mb-6"><LandingIcons.Shield w={40} /></div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2"><label className="text-sm font-black uppercase tracking-widest text-gray-400 ml-1">Email Address</label><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-6 py-4 bg-gray-50 border-2 border-transparent focus:border-[#EA580C] focus:bg-white rounded-[20px] outline-none transition-all font-bold" placeholder="name@example.com" required /></div>
          <div className="space-y-2"><label className="text-sm font-black uppercase tracking-widest text-gray-400 ml-1">Password</label><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-6 py-4 bg-gray-50 border-2 border-transparent focus:border-[#EA580C] focus:bg-white rounded-[20px] outline-none transition-all font-bold" placeholder="••••••••" required /></div>
          <button type="submit" className="w-full py-5 bg-[#111827] text-white rounded-full text-lg font-black shadow-2xl shadow-[#111827]/20 hover:shadow-[#111827]/40 hover:-translate-y-1 transition-all">Login to Dashboard</button>
        </form>
        <div className="mt-12 text-center text-sm font-bold text-gray-400">New to SAHARA? <button className="text-[#EA580C] hover:underline decoration-2">Create an account</button></div>
      </div>
    </div>
  );
};

// ─── Theme (Wait, I'll keep the theme where it is) ──────────────────────────
...
export default function SAHARA() {
    ...
    const score = (() => {
        ...
        // 🏃 Activity: Steps and Sleep targets
        if (steps < 4000) s -= 10; else if (steps < 6000) s -= 5;
        if (sleep < 6 || sleep > 9) s -= 7;

        // 🍎 Nutrition: Optimal calorie and protein intake
...
_Via SAHARA Health Companion · ${timeStr}_`);
    ...
    if (page === 'landing') {
        return <LandingPage onStart={() => setPage('dashboard')} onLogin={() => setPage('login')} />;
    }
        ...
        <main style={{ maxWidth: 1120, margin: "0 auto", padding: "24px 18px 180px", display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,480px),1fr))", gap: 18 }}>
            ...
            <Card th={th} full d={1} show={show}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(175px,1fr))", gap: 12, marginTop: 16 }}>
                    ...
                    { k: "heart", label: "Heart Rate", unit: "bpm", ic: <Ic.Heart w={24} />, tint: dark ? "#28100e" : "#fff1f2" }].map(v => (
                        <div key={v.k} style={{ borderRadius: 18, padding: "18px 16px", position: "relative", background: v.tint, border: `1.5px solid ${th.border}`, transition: "border-color .2s" }}>
                            <span style={{ color: G.green }}>{v.ic}</span>
                            ...
                            <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 9 }}>
                                <span style={{ width: 8, height: 8, borderRadius: "50%", background: G.green, display: "inline-block", boxShadow: `0 0 6px ${G.green}80` }} />
                                <span style={{ fontSize: 12, color: G.green, fontWeight: 600 }}>Normal</span>
                            </div>
                            {vMode === "manual" && (
                                <button className="b" onClick={() => { setEditV(v.k); setTmpV(vitals[v.k]); }}
            ...
            <Card th={th} d={5} show={show}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 11, marginTop: 15 }}>
                    <AlertRow th={th} dark={dark} sev="warn" title="Anaemia Risk Detected" desc="Preventive — consult doctor at next visit. Eat iron-rich foods like spinach and lentils." />
                    <AlertRow th={th} dark={dark} sev="info" title="Step Goal Below Target" desc="You are 36% below today's step goal. A short 10-minute walk is recommended." />
                </div>
            </Card>
                    ...
                    {vtxt && (
                        <div style={{ width: "100%", background: th.s2, border: `1.5px solid ${th.border}`, borderRadius: 13, padding: "12px 15px" }}>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 10 }}>
                                {vEntries.map((e, i) => (
                                    <div key={i} style={{ padding: "8px 12px", background: dark ? G.gBgD : G.gBg, borderRadius: 9, border: `1px solid ${dark ? G.gBdD : G.gBd}`, color: G.green, fontSize: 14, fontWeight: 700 }}>
                                        ✓ {e.name} ({e.kcal} kcal)
                                    </div>
                                ))}
                            </div>
