...
const LoginPage = ({ onBack }) => {
  const { login, googleSignIn, loginAsDemo } = useAuth();
    ...
    <div className="min-h-screen bg-[#F5F4F0] text-[#111827] font-['Outfit',sans-serif] flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md bg-white/90 backdrop-blur-md p-10 rounded-[40px] shadow-2xl border border-[#E4E2DB]">
        ...
        <div className="mt-4 grid grid-cols-2 gap-4">
          <button 
            onClick={() => {
              const user = loginAsDemo('senior');
              navigate('/senior');
            }}
            className="py-4 bg-[#EA580C]/10 text-[#EA580C] border-2 border-[#EA580C]/20 rounded-[24px] text-sm font-black hover:bg-[#EA580C]/20 transition-all"
          >
            Senior Demo
          </button>
          <button 
            onClick={() => {
              const user = loginAsDemo('family');
              navigate('/family');
            }}
            className="py-4 bg-[#111827]/5 text-[#111827] border-2 border-[#111827]/10 rounded-[24px] text-sm font-black hover:bg-gray-200 transition-all"
          >
            Family Demo
          </button>
        </div>
