...
function LoginPage({ onLogin }) {
    ...
    return (
        <div style={{ minHeight: "100vh", background: "#fff", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 24px", fontFamily: "'Outfit', sans-serif" }}>
            <style>{`
                @keyframes lu{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}
                .lu{animation:lu .38s cubic-bezier(.4,0,.2,1) both}
                .lgn-inp:focus{border-color:#131313 !important;background:#fff !important}
                .lgn-btn:hover:not(:disabled){background:#222 !important}
                .lgn-btn:active:not(:disabled){transform:scale(.98)}
                .lpill:hover{border-color:#131313 !important}
            `}</style>

            {/* Logo */}
            <div className="lu" style={{ animationDelay: "0s", marginBottom: 44, display: "flex", alignItems: "center", gap: 9 }}>
                <div style={{ width: 32, height: 32, borderRadius: 9, background: "#131313", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <Ic.Shield w={16} style={{ color: "#fff" }} />
                </div>
                <span style={{ fontSize: 17, fontWeight: 700, letterSpacing: "-.01em", color: "#131313" }}>SAHARA</span>
            </div>

            <div className="lu" style={{ animationDelay: ".05s", width: "100%", maxWidth: 420 }}>
                <h1 style={{ fontSize: 38, fontWeight: 800, color: "#131313", letterSpacing: "-.025em", lineHeight: 1.05, marginBottom: 7 }}>Welcome back.</h1>
                <p style={{ fontSize: 15, color: "rgba(0,0,0,.36)", marginBottom: 30, lineHeight: 1.5 }}>Sign in to your health companion.</p>

                {/* Quick-fill pills */}
                <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".1em", color: "rgba(0,0,0,.28)", textTransform: "uppercase", marginBottom: 10 }}>Quick access</p>
                <div style={{ display: "flex", gap: 8, marginBottom: 26, flexWrap: "wrap" }}>
                    {USERS.map(u => (
                        <button key={u.id} className="lpill" onClick={() => quickFill(u)}
                            style={{ display: "flex", alignItems: "center", gap: 7, padding: "7px 14px", borderRadius: 99, border: `1.5px solid ${fillUser === u.id ? "#131313" : "#E4E2DB"}`, background: fillUser === u.id ? "#131313" : "#fff", color: fillUser === u.id ? "#fff" : "#131313", fontFamily: "'Outfit',sans-serif", fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "all .15s" }}>
                            <span style={{ fontSize: 15 }}>{u.avatar}</span>
                            {u.name.split(" ")[0]}
                            <span style={{ fontSize: 11, opacity: .5, fontWeight: 400, textTransform: "capitalize" }}>{u.role}</span>
                        </button>
                    ))}
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".1em", color: "rgba(0,0,0,.24)" }}>OR</span>
                    <div style={{ flex: 1, height: 1, background: "#ECEAE3" }} />
                </div>

                <form onSubmit={handleLogin}>
                    <input id="login-email" className="lgn-inp" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Email address"
                        style={{ width: "100%", padding: "13px 16px", borderRadius: 12, border: "1.5px solid #E4E2DB", background: "#F5F4F0", fontSize: 15, fontFamily: "'Outfit',sans-serif", color: "#131313", outline: "none", marginBottom: 10, boxSizing: "border-box", transition: "all .15s" }} required />
                    <div style={{ position: "relative", marginBottom: 14 }}>
                        <input id="login-pass" className="lgn-inp" type={showPass ? "text" : "password"} value={pass} onChange={e => setPass(e.target.value)} placeholder="Password"
                            style={{ width: "100%", padding: "13px 56px 13px 16px", borderRadius: 12, border: "1.5px solid #E4E2DB", background: "#F5F4F0", fontSize: 15, fontFamily: "'Outfit',sans-serif", color: "#131313", outline: "none", boxSizing: "border-box", transition: "all .15s" }} required />
                        <button type="button" onClick={() => setShowPass(s => !s)} style={{ position: "absolute", right: 14, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "rgba(0,0,0,.32)", fontSize: 11, fontWeight: 700, fontFamily: "'Outfit',sans-serif", letterSpacing: ".06em" }}>{showPass ? "HIDE" : "SHOW"}</button>
                    </div>
                    {err && <div style={{ background: "#fff1f2", border: "1.5px solid #fecaca", color: "#dc2626", borderRadius: 10, padding: "9px 13px", fontSize: 13, fontWeight: 600, marginBottom: 12 }}>⚠️ {err}</div>}
                    <button id="login-submit" className="lgn-btn" type="submit" disabled={loading}
                        style={{ width: "100%", padding: "14px", borderRadius: 12, background: loading ? "#9B9890" : "#131313", color: "#fff", border: "none", fontSize: 15, fontWeight: 700, fontFamily: "'Outfit',sans-serif", cursor: loading ? "not-allowed" : "pointer", transition: "background .15s", letterSpacing: ".01em" }}>
                        {loading ? "Signing in…" : "Continue →"}
                    </button>
                </form>

                <div style={{ marginTop: 24, padding: "13px 15px", background: "#F5F4F0", borderRadius: 12, border: "1.5px solid #E4E2DB" }}>
                    <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".1em", color: "rgba(0,0,0,.3)", textTransform: "uppercase", marginBottom: 10 }}>Test credentials</p>
                    {USERS.map(u => (
                        <div key={u.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 7 }}>
                            <span style={{ fontSize: 13, color: "#131313", fontWeight: 600 }}>{u.avatar} {u.name} <span style={{ color: "rgba(0,0,0,.34)", fontWeight: 400, textTransform: "capitalize" }}>({u.role})</span></span>
                            <span style={{ fontSize: 12, color: "rgba(0,0,0,.4)", fontFamily: "monospace", background: "#fff", padding: "2px 8px", borderRadius: 6, border: "1px solid #E4E2DB" }}>{u.pass}</span>
                        </div>
                    ))}
                </div>
            </div>
            <p className="lu" style={{ animationDelay: ".12s", marginTop: 36, fontSize: 11, color: "rgba(0,0,0,.22)" }}>© 2025 SAHARA Health · Powered by Gemini AI</p>
        </div>
    );
}
...
function SAHARA({ currentUser, onLogout }) {
    ...
    const [view, setView] = useState(currentUser?.role === "family" ? "family" : "senior");
                ...
                <header style={{ background: th.hdr, backdropFilter: "blur(18px)", WebkitBackdropFilter: "blur(18px)", borderBottom: `1px solid ${th.border}`, position: "sticky", top: 0, zIndex: 50, boxShadow: th.shadow, transition: "background .35s" }}>
                    <div style={{ maxWidth: 1120, margin: "0 auto", padding: "0 20px" }}>
                        ...
                        <div style={{ display: "flex", gap: 8, paddingBottom: 13 }}>
                            ...
                            { id: "family", ic: <Ic.Family w={19} />, lbl: "For Family", sub: "Monitor & alerts", badge: unread || null }]
                            .filter(t => currentUser?.role === "family" ? t.id === "family" : t.id === "senior")
                            .map(t => (
            ...
            <Card th={th} full d={1} show={show}>
                ...
                {vMode === "auto" && (
                    <div style={{ marginTop: 13, display: "flex", alignItems: "center", gap: 8, color: th.muted, fontSize: 13 }}>
                        ...
                        Auto-monitoring active · Syncing every 5 minutes
                    </div>
                )}
            </Card>
