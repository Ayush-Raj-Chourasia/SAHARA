def calculate_anaemia_risk_basic(hb: float, gender: str, age: int, fatigue: int = 0) -> dict:
    """
...
async def calculate_anaemia_risk(hb: float, gender: str, age: int, fatigue: int = 0, history: list = None) -> dict:
    # Always attempt AI fallback for better "predictive" capability as requested by user
    if api_key:
        return await calculate_anaemia_risk_ai_fallback(hb, gender, age, fatigue, history)
    return calculate_anaemia_risk_basic(hb, gender, age, fatigue)
    """
    WHO anaemia clinical thresholds adjusted for Indian elderly (60+).
    Reference: WHO/NMH/NHD/MNM/11.1
    """
